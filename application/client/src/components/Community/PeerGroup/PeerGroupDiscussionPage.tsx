...
import {
  ...
  createGroupEvent,
  rsvpToGroupEvent,
  fetchNetworkingImpact,
  createJobFromPeerOpportunity,
...
export default function PeerGroupDiscussionPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (parsed?.user?._id) {
        setCurrentUserId(parsed.user._id);
      }
    } catch (err) {
  ...
  // ---- Jobs (for picking an existing job to share) ----
  ...
  const [oppInitialDraft, setOppInitialDraft] =
    useState<Partial<OpportunityDraft> | null>(null);
  ...
  // Add to tracker state (kept for future spinner use)
  const [addToTrackerLoadingId, setAddToTrackerLoadingId] =
    useState<string | null>(null);
  const [addToTrackerError, setAddToTrackerError] = useState<string | null>(
    null
  );
  ...
  const [activeSection, setActiveSection] =
    useState<SectionKey>("discussion");
  ...
  const interactionLabel = useMemo(() => {
    ...
    if (mode === "anonymous")
      return "Your posts appear as Anonymous in this group.";
    if (mode === "alias")
      return `Your posts appear under your alias "${
        membership.alias || "Anonymous"
      }".`;
  ...
  // Load group / posts / challenges / opportunities / events
  useEffect(() => {
    if (!groupId || !currentUserId) return;
  ...
  const handleAddOpportunityToTracker = async (opp: PeerOpportunity) => {
    ...
    try {
      ...
      console.error(e);
      setAddToTrackerError(
        e?.message ?? "Failed to add opportunity to job tracker."
      );
    } finally {
  ...
  const handlePickJobForOpportunity = (
    job: import("../../Coverletter/hooks/useJobs").Job
  ) => {
    ...
    try {
      ...
      await createGroupEvent({
        ...
        payload: {
          ...
          maxAttendees: eventMaxAttendees ? Number(eventMaxAttendees) : 0,
        },
      });
  ...
  const handleExpressInterest = async (opp: PeerOpportunity, note: string) => {
    ...
    try {
      await expressInterestInOpportunity(opp._id, currentUserId, {
        note,
        status: "interested",
      });
      const oppData = await fetchGroupOpportunities(groupId, currentUserId);
  ...
  const handleJoinChallenge = async (challenge: GroupChallenge) => {
    ...
    try {
      const participation = await joinGroupChallenge(
        challenge._id,
        currentUserId
      );
      setMyChallengeParticipations((prev) => {
        ...
        if (existing) {
          return prev.map((p) =>
            p._id === participation._id ? participation : p
          );
        }
  ...
  const handleUpdateChallengeProgress = async (
    ...
    try {
      const participation = await updateGroupChallengeProgress(
        challenge._id,
        currentUserId,
        {
          delta,
  ...
  const handleLeaveChallenge = async (challenge: GroupChallenge) => {
    if (!groupId || !currentUserId || !membership) return;
  ...
  const handleHighlightPost = async (
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((p) => (p._id === postId ? { ...p, highlightType } : p))
      );
    } catch (e) {
      console.error(e);
  ...
  const handlePost = async () => {
    if (!groupId || !currentUserId || !content.trim()) return;
    if (!membership) {
    ...
    try {
      ...
      await createGroupPost(groupId, currentUserId, {
        content,
        type: postType,
      });
  ...
  const handleSubmitPrivacy = async (values: GroupPrivacyFormValues) => {
    if (!groupId || !membership || !currentUserId) return;
    ...
    return (
      <div className="p-4">
        <div className="mx-auto max-w-5xl">
          ...
          <Button onClick={() => navigate("/peer-groups")}>
            Back to groups
          </Button>
        </div>
      </div>
  ...
  const renderSectionButton = (
    ...
    return (
      <button
        ...
        className={[
          "flex items-center gap-2 whitespace-nowrap rounded-full px-3 py-1 text-xs border",
          isActive
      ...
      >
        ...
        {badge && badge > 0 && (
          <span
            className={
              "inline-flex items-center justify-center rounded-full px-2 text-[10px] " +
              (isActive
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        ...
        <Card className="p-4 space-y-3">
          <div className="flex justify-between items-center">
            ...
            <select
              ...
              onChange={(e) =>
                setPostType(
                  e.target.value as "insight" | "question" | "strategy"
                )
              }
            >
          ...
          <div className="flex justify-end">
            <Button
              onClick={handlePost}
              disabled={!membership || posting || !content.trim()}
            >
        ...
        <div className="mt-2 border-b border-gray-200 pb-2">
          <div className="flex gap-2 overflow-x-auto no-scrollbar">
            {renderSectionButton("discussion", "Discussion")}
            {renderSectionButton("challenges", "Challenges")}
            {renderSectionButton(
              "opportunities",
              "Opportunities",
              newOppCount
            )}
            {renderSectionButton("events", "Events")}
            {renderSectionButton("impact", "Impact")}
          </div>
        </div>
            ...
            {(successPosts.length > 0 || learningPosts.length > 0) && (
              <Card className="p-3 space-y-2">
                <h2 className="text-sm font-medium">
                  Success stories & learning highlights
                </h2>
              ...
              {posts.map((post) => (
                <Card key={post._id} className="p-3 space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-xs inline-block rounded-full bg-gray-100 px-2 py-0.5 capitalize">
                      {post.type}
                    </span>
                    ...
                    {(isGroupOwner || post.isMine) && (
                      <div className="ml-auto flex gap-1">
                        <button
                          ...
                          onClick={() =>
                            handleHighlightPost(post._id, "success")
                          }
                        >
                          Mark success
                        </button>
                        <button
                          ...
                          className="text-[10px] underline text-gray-500"
                          onClick={() =>
                            handleHighlightPost(post._id, "learning")
                          }
                        >
                          Mark learning
                        </button>
        ...
        {activeSection === "challenges" && (
          <div className="space-y-2 mt-2">
            ...
            {!challengesLoading && challenges.length === 0 && (
              <p className="text-xs text-gray-500">
                No challenges yet. Group owners can create weekly goals like
                “Apply to 10 jobs” or “Send 5 networking messages”.
              </p>
            )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className="flex flex-col">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewChallengeStart(e.target.value)
                        }
                      />
                    </div>
                    <div className="flex flex-col">
          ...
          <div className="space-y-2 mt-2">
            ...
            {newOppCount > 0 && (
              <div className="flex items-center justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {newOppCount} new opportunit
                    {newOppCount === 1 ? "y" : "ies"} since your last visit to
                    this group.
                  </span>
                </div>
                <button
            ...
            {membership ? (
              <div className="border rounded-md p-3 bg-gray-50 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div className="text-xs text-gray-700">
                  ...
                  <p className="mt-1">
                    Post internships, new grad roles, and referral-ready
                    openings so the group can benefit.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
            ...
            <div className="space-y-2">
              {opportunities.length === 0 && !oppsLoading && (
                <p className="text-xs text-gray-500">
                  No opportunities shared yet. Once members start posting roles
                  and referrals, they’ll appear here.
                </p>
              )}
        ...
        {activeSection === "events" && (
          <div className="space-y-2 mt-2">
            ...
            {isGroupOwner ? (
              ...
              <p className="text-[11px] text-gray-500">
                Group owners can schedule coaching sessions and webinars for
                this group.
              </p>
            )}
            ...
            <div className="space-y-2">
              {events.length === 0 && !eventsLoading && (
                <p className="text-xs text-gray-500">
                  No sessions scheduled yet. Once the group owner adds events,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="border rounded-md p-3 bg-white">
                  <h3 className="text-sm font-medium mb-1">
                    Your outcomes from this group
                  </h3>
                  <p className="text-[11px] text-gray-500 mb-2">
                    Jobs you added to your tracker that came from shared
                    opportunities or referrals in this group.
                  </p>
                ...
                <div className="border rounded-md p-3 bg-white">
                  ...
                  <p className="text-[11px] text-gray-500 mb-2">
                    Combined results from jobs sourced through this group (all
                    members).
                  </p>
                  <div className="flex gap-4 text-sm">
                    <div>
                  ...
                  <p className="mt-2 text-[11px] text-gray-500">
                    ...
                    {networkingImpact.group.membersWithPeerJobs === 1
                      ? ""
                      : "s"}{" "}
                    have at least one job sourced from this group.
                  </p>
                </div>
              </div>
            )}
        ...
        {/* Job picker + mini opportunity form (shared across sections) */}
        <JobPickerSheet
