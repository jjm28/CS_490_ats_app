...
import type { ResumeData, TemplateKey, ContactInfo, FeedbackSummaryResponse } from "../../api/resumes";
import getAuthHeaders, { saveResume, updateResume, GetAiResumeContent, createSharedResume, getFullResume, updateResumeWorkflow, getResumeFeedbackSummary } from "../../api/resumes";
      ...
      const [reviewers, setReviewers] = useState<ReviewerMeta[]>([]);
        ...
        setInviteError(null);
          try {
              ...
              let inviteData = {
              toemail :inviteEmail, sharedurl: out.url, resumeId: resumeId, role: inviteRole, canComment: inviteCanComment, canResolve: inviteCanResolve, reviewDeadline: shareDeadline
              }
      ...
      const handleRemoveInvite = async (emailToRemove: string) => {
        setInvitedEmails((prev) => prev.filter((e) => e !== emailToRemove));
          setReviewers((prev) => prev.filter((r) => r.email !== emailToRemove));
              ...
              resumeid: resumeId, email :emailToRemove
              }
              const res = await fetch(
...
setReviewers(
  resp.reviewers?.map((r: { email: any; role: string | undefined; status: any; lastActivityAt: string | number | Date; completedAt: string | number | Date; }) => ({
    email: r.email,
  ...
  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
    ...
    <div>
      ...
      <p className="mt-1 text-sm text-gray-500">
        Fine-tune your resume content, run checks, and export to multiple formats.
      </p>
    </div>
