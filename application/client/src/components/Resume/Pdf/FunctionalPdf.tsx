...
export default function FunctionalPdf(props: ResumeData) {
  const { name, title, email, phone, location, summary, skills, projects, experience, education } = props;
  ...
  return (
    <Document>
      <Page size="A4" style={[tw("p-12 bg-white"), styles.base]}>
        ...
        {summary && (
          <View style={tw("mb-6")}>
            <Text style={[tw("text-[10px] uppercase tracking-wide mb-1"), styles.heading]}>Profile</Text>
        ...
        {!!skills?.length && (
          <View style={tw("mb-6")}>
            <Text style={[tw("text-[10px] uppercase tracking-wide mb-1"), styles.heading]}>Core Skills</Text>
        ...
        {!!projects?.length && (
          <View style={tw("mb-6")}>
            <Text style={[tw("text-[10px] uppercase tracking-wide mb-1"), styles.heading]}>Selected Projects</Text>
            {projects.map((p, i) => (
        ...
        {!!experience?.length && (
          <View style={tw("mb-6")}>
            ...
            {experience.map((job, i) => (
              <View key={i} style={[tw("mb-2 p-2"), { borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 4 }]}>
                <Text style={tw("text-[12px] font-semibold")}>{job.role}</Text>
                <Text style={tw("text-[11px] text-gray-700")}>{job.company}</Text>
                <Text style={tw("text-[10px] text-gray-500")}>{job.start} – {job.end}</Text>
              </View>
            ))}
          </View>
        )}
